...
SarsaLearner::SarsaLearner(ALEInterface& ale, Features *features, Parameters *param,int seed) : RLLearner(ale, param,seed) {
	...
	if (checkPointToLoad.is_open()){
        ...
        nameForLearningCondition = checkPointName+"-learningCondition-Frames"+std::to_string((long long) saveThreshold)+"-finished.txt";
...
void SarsaLearner::saveCheckPoint(int episode, int totalNumberFrames, vector<float>& episodeResults,int& frequency,vector<int>& episodeFrames, vector<double>& episodeFps){
    ofstream learningConditionFile;
    string newNameForLearningCondition = checkPointName+"-learningCondition-Frames"+std::to_string((long long) saveThreshold)+"-writing.txt";
    ...
    string currentCheckPointName = checkPointName+"-checkPoint-Frames"+std::to_string((long long) saveThreshold)+"-writing.txt";
    ...
    string previousVersionCheckPoint = checkPointName+"-checkPoint-Frames"+std::to_string((long long) saveThreshold-saveWeightsEveryXFrames)+"-finished.txt";
...
void SarsaLearner::loadCheckPoint(std::ifstream& checkPointToLoad){
...
void SarsaLearner::learnPolicy(ALEInterface& ale, Features *features){
	...
	for(int episode = episodePassed+1; episode < 5001; episode++){
